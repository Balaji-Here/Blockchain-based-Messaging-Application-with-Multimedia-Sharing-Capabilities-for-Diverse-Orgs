...
contract MainContract {
    ...
    function getApprovedOrRequestedMember(
        bool request
    ) public view returns (MemberStruct[] memory) {
        if (request == true) {
    ...
    // function to add or request member (true = request member | false = directly add member | ROLE1 will be directly added as a member)
    function addOrRequestMember(
        ...
        if (
            ...
            keccak256(abi.encodePacked("ROLE1"))
        ) {
            instanceOfMemberStruct.power = 1;
        } else if (
            keccak256(abi.encodePacked(_post)) ==
            keccak256(abi.encodePacked("ROLE2"))
        ) {
            instanceOfMemberStruct.power = 2;
        } else if (
            keccak256(abi.encodePacked(_post)) ==
            keccak256(abi.encodePacked("ROLE3"))
        ) {
            instanceOfMemberStruct.power = 3;
        } else if (
            keccak256(abi.encodePacked(_post)) ==
            keccak256(abi.encodePacked("ROLE4"))
        ) {
    ...
    function approoveRequest(
        address _userAddress,
        address approvingUserAddress
    ) public {
